...
export default function Nav() {
    ...
    return (
        <nav ref={navRef} style={{ transition: "height 100ms linear, padding 100ms linear", }} className="font-bold sm:h-6 md:h-8 flex sm:w-full sm:pl-1 sm:bg-gray-900 md:bg-white h-full md:outline outline-1 shadow-lg border-stone-300 outline-slate-100">
            <div ref={navInnerRef} className="flex justify-between items-center h-full w-full md:container mx-auto my-auto ">
                <div className={mobileMenuOpen ? "flex flex-col gap-6 items-start" : "flex items-center"}>
                    ...
                    <div ref={navlinksRef} className="gap-6 flex my-auto sm:hidden sm:flex-col md:flex md:flex-row md:text-md md:text-black sm:text-white">
                    ...
                    <div ref={menuIconRef}>
                        <AiOutlineMenu onClick={() => {
                            ...
                            navRef.current?.classList.toggle('sm:h-60')
