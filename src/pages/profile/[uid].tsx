      ...
      <div className="flex items-center justify-center p-6">
        <div className="flex flex-col mx-auto">
          ...
          {user.shares.map((post: any) => (
            <div
              key={post.id}
              className="text-center text-xl p-3 gap-2 flex items-center justify-center flex-col"
            >
              {post.title}
              <User
                post={post}
                userData={findUserData(post.authorId)}
                includeLikesAndShares={false}
              />
            </div>
          ))}
        </div>
