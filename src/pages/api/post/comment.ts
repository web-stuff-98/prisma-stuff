...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === "POST" || req.method === "PATCH") {
    try {
      await Yup.object()
        .shape({
          comment: Yup.string().max(200).notRequired(),
          postId: Yup.string().max(100).notRequired(),
          commentId: Yup.string().max(100).notRequired(),
        })
        .strict()
        .validate(req.body);
    } catch (e: any) {
  ...
  try {
    ...
    if (req.method === "PATCH" || req.method === "POST") {
      if (!comment || comment.trim() === "")
        return res.status(400).json({ msg: "Cannot submit an empty comment" });
      ...
      if (
        q?.comments.find(
          (comment: CommentOnPost) =>
            comment.userId === session?.uid &&
            comment.comment === req.body.comment
        )
      ) {
        return res
          .status(400)
          .json({ msg: "You have already made that comment" });
      }
      if (req.method === "POST") {
        ...
        if (!commentId) {
          ...
          await prisma.post.update({
            ...
            data: {
              comments: {
                create: { comment, userId: String(session?.uid), id },
              },
            },
          });
          await pusher.trigger(`post=${postId}`, "comment-added", {
            comment,
            userId: session?.uid,
          ...
          await pusher.trigger(`post=${postId}`, "comment-on-comment-added", {
            comment,
            userId: session?.uid,
      ...
      if (req.method === "PATCH") {
        if (commentId) {
          ...
          await pusher.trigger(`post=${postId}`, "comment-updated", {
            comment,
            userId: session?.uid,
            commentId,
          });
          ...
          await pusher.trigger(`post=${postId}`, "comment-on-comment-updated", {
            comment,
            userId: session?.uid,
            commentThreadId: commentId,
            id: commentId,
